...
public function create_new_details(Request $request){
   ...
   $imageName = time() . '.' . $request->img->extension(); $request->img->move(public_path('images'), $imageName);
...
public function showproduct_details(){
   ...
   ->get();
   return view ('dashboard.product_details',['prod'=>$prod,'productdetails'=>$productdetails]);
}
}